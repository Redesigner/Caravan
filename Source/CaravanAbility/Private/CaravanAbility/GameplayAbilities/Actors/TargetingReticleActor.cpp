...
void ATargetingReticleActor::Tick(float DeltaTime)
{
	...
	case EReticleMovementMode::FollowOnwer:
	{
		...
		{
			UE_LOG(LogTargetingReticleSystem, Warning, TEXT("Unable to find valid target for reticle."));
		}
		break;
	}
	case EReticleMovementMode::FollowInput:
	{
		...
		{
			UE_LOG(LogTargetingReticleSystem, Warning, TEXT("%s reticle '%s' unable to find owner."),
...
void ATargetingReticleActor::OnRep_SetOwningPawn()
{
	if (OwningPawn)
	{
		...
		{
			UE_LOG(LogTargetingReticleSystem, Warning, TEXT("%s Attempting to set InputComponent on 'BeginPlay' for reticle '%s', but already has an InputComponent"),
				GetNetMode() == ENetMode::NM_Client ? TEXT("[client]") : TEXT("[server]"),
				*GetName());
		}
	}
	...
	{
		UE_LOG(LogTargetingReticleSystem, Warning, TEXT("%s Attempting to set InputComponent for reticle '%s', but pawn was invalid."),
			GetNetMode() == ENetMode::NM_Client ? TEXT("[client]") : TEXT("[server]"),
			*GetName());
	}
}
